...
class ImageAug3D:
    def __init__(
        self, final_dim, resize_lim, bot_pct_lim, rot_lim, rand_flip=False, is_train=False
    ):
    ...
    def __call__(self, data):
        ...
        translations = []
        scales = []
        if 'semantic_img' in data:
            new_semantic_imgs = []
            semantic_imgs = data['semantic_img']
        for i, img in enumerate(imgs):
            resize, resize_dims, crop, flip, rotate = self.sample_augmentation(data)
            post_rot = torch.eye(2)
            post_tran = torch.zeros(2)
            new_img = self.img_transform(
                img,
                post_rot,
                post_tran,
                resize=resize,
            ...
            translations.append(torch.FloatTensor([crop[0], crop[1], 0]))
            scales.append(resize)
            new_imgs.append(new_img)
            if 'semantic_img' in data:
                new_semantic_img = self.img_transform(
                    semantic_imgs[i],
                    post_rot,
                    post_tran,
                    resize=resize,
                ...
                new_semantic_imgs.append(new_semantic_img)
        data["img"] = new_imgs
        # update the calibration matrices
        data["img_scale"] = torch.FloatTensor(scales)
        data['img_translation'] = torch.stack(translations, dim=0)
        if 'semantic_img' in data:
            data['semantic_img'] = new_semantic_imgs
        return data
...
class GlobalRotScaleTrans(object):
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        input_dict['pcd_trans'] = torch.from_numpy(trans_factor)
        for key in input_dict['bbox3d_fields']:
