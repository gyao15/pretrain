...
class ContrastiveFeatureHead(nn.Module):
    def __init__(self, num_conv, num_heads, num_classes, dropout=0.1, t=0.2, use_gt=False):
        ...
        self.use_gt = use_gt
        if use_gt:
            self.k_layers_ = nn.ModuleList()
            self.q_layers_ = nn.ModuleList()
            self.v_layers_ = nn.ModuleList()
            for i in range(self.num_classes):
                self.k_layers_.append(nn.Conv1d(num_conv, num_conv, 1, 1))
                self.q_layers_.append(nn.Conv1d(num_conv, num_conv, 1, 1))
                self.v_layers_.append(nn.Conv1d(num_conv, num_conv, 1, 1))
    ...
    def forward(self, query, key, key_=None):
        outs = []
        sims = []
        ...
        for i in range(self.num_classes):
            ...
            q = q.view(batch_size, self.num_heads, self.num_conv//self.num_heads, -1)
            k = k.view(batch_size, self.num_heads, self.num_conv//self.num_heads, -1)
            v = v.view(batch_size, self.num_heads, self.num_conv//self.num_heads, -1)
            sim = torch.matmul(q.transpose(2, 3), k) / self.t
            sim = torch.softmax(sim, dim=2)
            out = torch.sum(sim.unsqueeze(2) * v.unsqueeze(3), dim=-1)
            out = out.reshape(batch_size, self.num_conv, -1)
            sim = sim.reshape(batch_size, -1, v.shape[-1])
        ...
        if self.use_gt and key_ is not None:
            outs_ = []
            sims_ = []
            for i in range(self.num_classes):
                q = self.q_layers_[i](query[..., i])
                k = self.k_layers_[i](key_)
                v = self.v_layers_[i](key_)
                q = q.view(batch_size, self.num_heads, self.num_conv//self.num_heads, -1)
                k = k.view(batch_size, self.num_heads, self.num_conv//self.num_heads, -1)
                v = v.view(batch_size, self.num_heads, self.num_conv//self.num_heads, -1)
                sim = torch.matmul(q.transpose(2, 3), k) / self.t
                ...
                outs_.append(out)
                sims_.append(sim)
            outs_ = torch.stack(outs_, dim=-1)
            sims_ = torch.stack(sims_, dim=-1)
            outs_ = query + self.drop1(outs_)
            outs_ = self.out_layer(self.bn1(outs_))
            outs_ = query + self.drop2(outs_)
            outs_ = self.bn2(outs_)
            return outs, sims, outs_, sims_
        return outs, sims
...
class SFFN(nn.Module):
    def __init__(self,
        ...
        for head in self.heads:
            ...
            if head == 'heatmap':
                self.__setattr__(head+'_embed', nn.Parameter(torch.randn(head_conv, num_bins+1, classes), requires_grad=True))
            else:
                self.__setattr__(head+'_embed', nn.Parameter(torch.randn(head_conv, num_bins, classes), requires_grad=True))
            decoder = nn.ModuleList()
            for i in range(num_decoder):
                decoder.append(
                    ContrastiveFeatureHead(head_conv, num_heads, classes, use_gt=self.use_gt))
                ...
                if head == 'heatmap':
                    encoder.append(
                        GPTransformerLayer(
                            ...
                            self_posembed=None, kernel=kernel, unit=unit
                        )
                    )
                else:
                    encoder.append(
    ...
    def forward(self, x, preds_dict=None, gt_dicts=None):
        """Forward function for SepHead.
        ...
        for head in self.heads:
            ...
            if gt_dicts is not None:
                gt_embed = self.__getattr__(head+'_gt_layer')(gt_dicts[head])
            else:
                gt_embed = None

            if self.use_gt and (gt_embed is not None):
                for i in range(len(self.__getattr__(head+'_decoder'))):
                    statistic_embed, sims, statistic_embed_, sims_ = self.__getattr__(head+'_decoder')[i](statistic_embed, self.with_embed(head_related_features, head_embed), self.with_embed(head_related_features, gt_embed))
                ret_dict[head+'_sims'] = sims
                ret_dict[head+'_sims_q'] = sims_
                statistic_embed = statistic_embed.view(batch_size, -1, num_bins*num_classes)
                statistic_embed_ = statistic_embed_.view(batch_size, -1, num_bins*num_classes)
            else:
                for i in range(len(self.__getattr__(head+'_decoder'))):
            ...
            if self.use_gt and (gt_embed is not None):
                for i in range(len(self.__getattr__(head+'_encoder'))):
                    head_related_features_, sigma_ = self.__getattr__(head+'_encoder')[i](statistic_embed_, head_related_features, num_classes)
                ret_dict[head+'_mean_q'] = self.__getattr__(head)(head_related_features_)
                ret_dict[head+'_sigma_q'] = sigma_

            for i in range(len(self.__getattr__(head+'_encoder'))):
                head_related_features, sigma = self.__getattr__(head+'_encoder')[i](statistic_embed, head_related_features, num_classes)
...
class GaussianProcess(nn.Module):
    def __init__(self, embed_dim, num_heads, bias=True, kernel='dot', unit=False, noise=False):
    ...
    def forward(self, head_embed, features, num_classes):
        ...
        if self.kernel == 'dot':
            ...
            if self.unit:
                sigma = torch.sum(torch.pow(sim, 2), dim=2)
            else:
                #Kkk = torch.exp(-torch.linalg.norm(k, dim=-1))
                sigma = torch.sum(torch.pow(sim, 2), dim=2)
        ...
        out = torch.einsum('bnmkh,bmkhc->bnkhc', sim, v)
        noise = torch.randn_like(out)
        out = out + sigma.unsqueeze(-1) * noise
        out = out.contiguous().view(batch_size, N, num_classes, -1)
        #out = torch.sum(out, dim=2)
        out = self.out_proj(out)
        return out, sigma

class GPTransformerLayer(nn.Module):
    def __init__(self, embed_dim, num_heads, ffn_channel, dropout=0.0, activation='relu', self_posembed=None, bias=True, kernel='dot', unit=True):
        super().__init__()
        self.gp = GaussianProcess(embed_dim, num_heads, bias=bias, kernel=kernel, unit=unit, noise=False)
    ...
    def forward(self, head_embed, features, num_classes):
        ...
        feat, sigma = self.gp(head_embed, self.with_pos_embed(features, pos_emb), num_classes)
...
class STransFusionHeadv3(nn.Module):
    ...
    def forward_single(self, inputs, img_inputs, img_metas, gt_bboxes_3d=None, gt_labels_3d=None):
        ...
        if self.initialize_by_heatmap:
            if self.statistic_cfg['heatmap_head']:
                ...
                pre_dict = self.headmap_head(dense_heatmap_flatten, None)
                #print(dense_heatmap)
                dense_heatmap = pre_dict['heatmap_mean'].view(batch_size, self.num_classes, lidar_feat.shape[-2], lidar_feat.shape[-1])
            else:
        ...
        for i in range(self.num_decoder_layers):
            ...
            second_res_layer = self.prediction_heads[i](query_feat, first_res_layer, query_gt_dict)
            res_layer.update(second_res_layer)
            # for next level positional embedding
            res_layer['center'] = res_layer['center'] + query_pos.permute(0, 2, 1)
            res_layer['center_mean'] = res_layer['center_mean'] + query_pos.permute(0, 2, 1)
            if self.training and self.use_gt:
                res_layer['center_mean_q'] = res_layer['center_mean_q'] + query_pos.permute(0, 2, 1)
        ...
        if self.initialize_by_heatmap:
            ...
            if self.statistic_cfg['heatmap_head'] is not None:
                ret_dicts[0]['dense_heatmap_sigma'] = pre_dict['heatmap_sigma']
            if self.fuse_img:
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for idx_layer in range(self.num_decoder_layers if self.auxiliary else 1):
            ...
            layer_score_p = preds_dict['heatmap_mean'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
            ...
            if self.use_gt:
                ...
                layer_cls_sims = preds_dict['heatmap_sims_q']
                layer_loss_contrast_cls = (-layer_cls_sims * (layer_cls_sims + 1e-3).log()).sum(dim=1).sum(dim=-1)
                layer_loss_contrast_cls_q = (layer_loss_contrast_cls * layer_label_weights).sum() / max(num_pos, 1)
            ...
            if self.use_gt:
                layer_center_sims = preds_dict['center_sims_q']
                #print(layer_center_sims.shape)
                eps = 1e-3
                layer_loss_contrast_center = (layer_center_sims * (layer_center_sims + eps).log()).sum(dim=1)
                layer_height_sims = preds_dict['height_sims_q']
                layer_loss_contrast_height = (layer_height_sims * (layer_height_sims + eps).log()).sum(dim=1)
                layer_dim_sims = preds_dict['dim_sims_q']
                layer_loss_contrast_dim = (layer_dim_sims * (layer_dim_sims + eps).log()).sum(dim=1)
                layer_rot_sims = preds_dict['rot_sims_q']
                layer_loss_contrast_rot = (layer_rot_sims * (layer_rot_sims + eps).log()).sum(dim=1)
                layer_loss_contrast_reg_q = torch.cat([layer_loss_contrast_center, layer_loss_contrast_height, layer_loss_contrast_dim, layer_loss_contrast_rot], dim=-1)
                layer_loss_contrast_reg_q = (-layer_loss_contrast_reg_q * layer_reg_weights).sum() / max(num_pos, 1)
            ...
            if self.loss_bbox_type == 'L1Loss':
                ...
                layer_loss_pbox = self.loss_bbox(preds_p, layer_bbox_targets, layer_reg_weights, avg_factor=max(num_pos, 1))
                #loss_dict[f'{prefix}_loss_reg_sigma'] = torch.sum(preds_var * layer_reg_weights) / max(num_pos, 1) * 0.2

                if self.use_gt:
                    layer_center = preds_dict['center_mean_q'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                    layer_height = preds_dict['height_mean_q'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                    layer_rot = preds_dict['rot_mean_q'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                    layer_dim = preds_dict['dim_mean_q'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                    preds_q = torch.cat([layer_center, layer_height, layer_dim, layer_rot], dim=1).permute(0, 2, 1)  # [BS, num_proposals, code_size]

                    #layer_center_var = preds_dict['center_sigma_q'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                    #layer_height_var = preds_dict['height_sigma_q'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                    #layer_rot_var = preds_dict['rot_sigma_q'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                    #layer_dim_var = preds_dict['dim_sigma_q'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                    #preds_var = torch.cat([layer_center_var, layer_height_var, layer_dim_var, layer_rot_var], dim=1).permute(0, 2, 1)

                    layer_loss_qbox = self.loss_bbox(preds_q, layer_bbox_targets, layer_reg_weights, avg_factor=max(num_pos, 1))
                    #loss_dict[f'{prefix}_loss_reg_sigma_q'] = torch.sum(preds_var * layer_reg_weights) / max(num_pos, 1) * 0.2
            ...
            if self.use_gt:
                ...
                #loss_dict[f'{prefix}_loss_cls_sigma_q'] = torch.mean(preds_dict['heatmap_sigma_q']) * 0.2
                loss_dict[f'{prefix}_loss_contrast_cls_q'] = layer_loss_contrast_cls_q * 0.2
                loss_dict[f'{prefix}_loss_contrast_reg_q'] = layer_loss_contrast_reg_q * 0.2
                #print(loss_dict)
