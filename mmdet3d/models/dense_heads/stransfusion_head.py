...
class ContrastiveAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, num_classes, proj_class_agnostic=True, attn_class_agnostic=False, dropout=0., bias=True):
        ...
        self.bias = bias
        self.proj_class_agnostic = proj_class_agnostic
        self.attn_class_agnostic = attn_class_agnostic

        self.num_classes = num_classes
        self.in_proj_q = nn.Linear(embed_dim, embed_dim, bias=bias)
        if self.proj_class_agnostic:
            self.in_proj_kv = nn.Linear(embed_dim, 2 * embed_dim, bias=bias)
        else:
            self.in_proj_kv = nn.ModuleList()
            for _ in range(num_classes):
                self.in_proj_kv.append(nn.Linear(embed_dim, 3 * embed_dim, bias=bias))
    ...
    def _reset_parameters(self):
        ...
        if self.proj_class_agnostic:
            ...
            for m in self.in_proj_kv:
                xavier_uniform_(m.weight)
        if self.bias:
            constant_(self.in_proj_q.bias, 0.)
            if self.proj_class_agnostic:
                constant_(self.in_proj_kv.bias, 0.)
            else:
                for m in self.in_proj_kv:
                    constant_(m, 0.)
            constant_(self.out_proj.bias, 0.)

    def forward(self, bev_features_flatten, obj_features, scores, labels, dists=None):
        batch_size, N, embed_dim = bev_features_flatten.size()
        _, nc, nd, ni, ns, _ = obj_features.size()
        q = self.in_proj_q(bev_features_flatten)

        #print(bev_features_flatten[0, 0])
        #print(obj_features[0])
        if self.proj_class_agnostic:
            kv = self.in_proj_kv(obj_features)
        else:
            kv = []
            for c in range(nc):
                kv.append(self.in_proj_kv[c](obj_features[c]))
            kv = torch.stack(kv, dim=0)

        k, v = kv.chunk(2, dim=-1)
        q = q.contiguous().view(batch_size, N, self.num_heads, self.head_dim)
        k = k.contiguous().view(batch_size, nc, nd*ni*ns, self.num_heads, self.head_dim)
        v = v.contiguous().view(batch_size, nc, nd*ni*ns, self.num_heads, self.head_dim)
        if not self.attn_class_agnostic:
            attn_feats = torch.zeros_like(q)
            for b in range(batch_size):
                for i in range(self.num_classes):
                    mask = (labels[b] == i)
                    if mask.sum() > 0:
                        score = scores[b][mask][:, i]
                        score = score.view(-1, 1, 1, 1, 1)
                        score = torch.cat([score, (1-score)*score, -score], dim=2).repeat(1, nd, 1, ns, self.num_heads)
                        score = score.contiguous().view(-1, nd*ni*ns, self.num_heads)
                        sim = torch.einsum('nhc,mhc->nmh', q[b, mask], k[b, i])
                        prob = torch.softmax(sim / self.scale, dim=1)
                        prob = prob * score
                        attn_feats[b][mask] = torch.einsum('nmh,mhc->nhc', prob, v[b, i])
        else: 
            raise NotImplementedError
        
        return attn_feats.contiguous().view(batch_size, N, -1)

class ContrastiveTransformerLayer(nn.Module):
    def __init__(self, embed_dim, num_heads, num_classes, proj_class_agnostic=True, attn_class_agnostic=False, dropout=0.1, bias=True, ffn_ratio=4, activation='relu'):
        super().__init__()
        self.contrastive_attn = ContrastiveAttention(embed_dim, num_heads, num_classes, proj_class_agnostic=proj_class_agnostic, attn_class_agnostic=attn_class_agnostic, dropout=dropout, bias=bias)
        self.linear1 = nn.Linear(embed_dim, embed_dim*ffn_ratio)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(embed_dim*ffn_ratio, embed_dim)
    ...
    def forward(self, query, key, query_pos, key_pos, scores, labels, dists=None):
        """
        :param query: B C Pq
        :param key: B Pk C
        :param query_pos: B Pq 3/6
        ...
        feat = self.contrastive_attn(self.with_pos_embed(query, query_pos), self.with_pos_embed(key, key_pos), scores, labels, dists)
        query = query + self.dropout2(feat)
        query = self.norm2(query)
...
class STransFusionHead(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(STransFusionHead, self).__init__()
        ...
        self.niou = len(self.statistic_cfg['iou'])
        self.ndist = len(self.statistic_cfg['dist'])
        self.register_buffer('obj_mean', torch.zeros(self.num_classes, self.ndist, self.niou, self.hidden_channel))
        self.register_buffer('obj_var', torch.zeros(self.num_classes, self.ndist, self.niou, self.hidden_channel))
        #self.register_buffer('bg_mean', torch.zeros(self.hidden_channel))
        #self.register_buffer('bg_var', torch.zeros(self.hidden_channel))
        self.register_buffer('temp_mean', torch.zeros(self.num_classes, self.ndist, self.niou, self.hidden_channel))
        self.register_buffer('temp_var', torch.zeros(self.num_classes, self.ndist, self.niou, self.hidden_channel))
        #self.register_buffer('temp_bg_mean', torch.zeros(self.hidden_channel))
        #self.register_buffer('temp_bg_var', torch.zeros(self.hidden_channel))
        self.register_buffer('temp_N', torch.zeros(self.num_classes, self.ndist, self.niou))

        self.register_buffer('iter', torch.LongTensor([0]))

        if self.statistic_cfg['preload_path'] is not None:
            m = mmcv.load(os.path.join(self.statistic_cfg['preload_path'], 'mean.pkl'))[0]
            var = mmcv.load(os.path.join(self.statistic_cfg['preload_path'], 'var.pkl'))[0]
            self.obj_mean = torch.FloatTensor(m)
            self.obj_var = torch.FloatTensor(var)
            #self.bg_mean = torch.FloatTensor(pre['bg_mean'])
            #self.bg_var = torch.FloatTensor(pre['bg_var'])
        
        self.encoder = nn.ModuleList()
        self.cls_emb = nn.Parameter(torch.Tensor(self.num_classes, self.hidden_channel))
        self.dist_emb = nn.Parameter(torch.Tensor(self.ndist, self.hidden_channel))
        self.iou_emb = nn.Parameter(torch.Tensor(self.niou, self.hidden_channel))
        self.bev_emb = PositionEmbeddingLearned(2, hidden_channel)
        for i in range(self.statistic_cfg['n_layers']):
            self.encoder.append(
                ContrastiveTransformerLayer(
                    hidden_channel, 8, num_classes, proj_class_agnostic=self.statistic_cfg['proj_class_agnostic'], 
                    attn_class_agnostic=self.statistic_cfg['attn_class_agnostic'], dropout=dropout, bias=True, 
                    ffn_ratio=self.statistic_cfg['ffn_ratio'], activation=activation
                ))
            

        if self.statistic_cfg['pre_classification']:
            layers = []
    ...
    def get_obj_emb(self):
        obj_emb = self.cls_emb.view(self.num_classes, 1, 1, -1) + self.dist_emb.view(1, self.ndist, 1, -1) + self.iou_emb.view(1, 1, self.niou, -1)
        return obj_emb

    def sample_obj_features(self, batch_size):
        x = torch.randn(batch_size, self.num_classes, self.ndist, self.niou, int(self.statistic_cfg['n_sample']), self.obj_mean.shape[-1])
        x = x.type_as(self.obj_mean)
        x = torch.clamp(x, min=-1, max=1)
        x = x * self.obj_var.unsqueeze(-2) + self.obj_mean.unsqueeze(-2)
        return x

    @staticmethod
    def cal_statistic(feature, score):
        if feature.shape[0] > 0:
            m = torch.sum(feature * score.unsqueeze(-1), dim=0) / score.sum()
            v = torch.sum(score.unsqueeze(-1)* (feature - m)**2, dim=0) / score.sum()
        else:
            m = feature.new_zeros(feature.shape[1])
            v = feature.new_zeros(feature.shape[1])
        return m, v
    
    def cal_dist_statistic(self, batch_features, scores, batch_iou, labels, mask):
        out_m, out_var = [], []
        nums = batch_features.new_zeros(self.num_classes, self.niou)
        for i in range(self.num_classes):
            cls_mask = (labels == i) & mask
            pos_mask = cls_mask & (batch_iou >= self.statistic_cfg['iou_threshold'][i])
            in_mask = cls_mask & (batch_iou < self.statistic_cfg['iou_threshold'][i]) & (batch_iou > 0)
            fp_mask = cls_mask & (batch_iou <= 0)
            pos_m, pos_var = STransFusionHead.cal_statistic(batch_features[pos_mask], scores[pos_mask])
            in_m, in_var = STransFusionHead.cal_statistic(batch_features[in_mask], scores[in_mask])
            fp_m, fp_var = STransFusionHead.cal_statistic(batch_features[fp_mask], scores[fp_mask])
            out_m.append(torch.stack([pos_m, in_m, fp_m], dim=0))
            out_var.append(torch.stack([pos_var, in_var, fp_var], dim=0))
            nums[i, 0] = pos_mask.sum()
            nums[i, 1] = in_mask.sum()
            nums[i, 2] = fp_mask.sum()
        out_m = torch.stack(out_m, dim=0)
        out_var = torch.stack(out_var, dim=0)
        return out_m, out_var, nums

    def update_temp_params(self, preds_dicts, gt_ious=None):
        preds_dict = preds_dicts[0]

        batch_size = preds_dict[0]['heatmap'].shape[0]
        batch_score = preds_dict[0]['heatmap'][..., -self.num_proposals:].detach().sigmoid()
        batch_iou = preds_dict[0]['iou'][..., -self.num_proposals:].detach().sigmoid() if gt_ious is None else gt_ious[:, -self.num_proposals:]
        #print(batch_score.shape, batch_iou.shape)
        #if self.loss_iou.loss_weight != 0:
        
        one_hot = F.one_hot(self.query_labels, num_classes=self.num_classes).permute(0, 2, 1)
        batch_score = batch_score * preds_dict[0]['query_heatmap_score'].detach() * one_hot

        batch_center = preds_dict[0]['center'][..., -self.num_proposals:].detach()
        batch_height = preds_dict[0]['height'][..., -self.num_proposals:].detach()
        batch_dim = preds_dict[0]['dim'][..., -self.num_proposals:].detach()
        batch_rot = preds_dict[0]['rot'][..., -self.num_proposals:].detach()

        batch_vel = None
        if 'vel' in preds_dict[0]:
            batch_vel = preds_dict[0]['vel'][..., -self.num_proposals:].detach()
        batch_features = None
        if 'features' in preds_dict[0]:
            batch_features = preds_dict[0]['features'][..., -self.num_proposals:].transpose(1, 2)

        temp, mask = self.bbox_coder.decode(batch_score, batch_rot, batch_dim, batch_center, batch_height, batch_vel, filter=True, return_mask=True)
        scores = []
        dists = []
        labels = []
        for b in range(batch_size):
            labels.append(temp[b]['labels'])
            scores.append(temp[b]['scores'])
            dists.append(torch.linalg.norm(temp[b]['bboxes'][..., :2], dim=-1))
        scores = torch.cat(scores, dim=0)
        dists = torch.cat(dists, dim=0)
        labels = torch.cat(labels, dim=0)
        #print(mask.shape)
        batch_features = batch_features[mask]
        batch_iou = batch_iou[mask]
        #batch_features = batch_features.contiguous().view(-1, batch_features.shape[-1])
        #batch_iou = batch_iou.view(-1)
        pos_mask = batch_iou > 0
        #print(scores.shape, batch_score.shape, batch_iou.shape)
        scores[pos_mask] = torch.sqrt(scores[pos_mask] * batch_iou[pos_mask])
        scores[~pos_mask] = 1 - scores[~pos_mask]

        mask = (dists < 30)
        near_m, near_var, near_num = self.cal_dist_statistic(batch_features, scores, batch_iou, labels, mask)
        mask = (dists >= 25) & (dists < 55)
        mid_m, mid_var, mid_num = self.cal_dist_statistic(batch_features, scores, batch_iou, labels, mask)
        mask = (dists >= 50)
        far_m, far_var, far_num = self.cal_dist_statistic(batch_features, scores, batch_iou, labels, mask)

        cur_mean = torch.stack([near_m, mid_m, far_m], dim=1)
        cur_var = torch.stack([near_var, mid_var, far_var], dim=1)
        cur_num = torch.stack([near_num, mid_num, far_num], dim=1)
        
        N = torch.clamp(self.temp_N + cur_num, min=1.0)
        momentum = (self.temp_N / N).unsqueeze(-1)
        #print(self.temp_mean.shape, self.temp_var.shape, cur_var.shape)
        self.temp_var = momentum * self.temp_var + (1-momentum) * cur_var + (momentum**2 + (1-momentum)**2) * (self.temp_mean - cur_mean)**2
        self.temp_mean = momentum * self.temp_mean + (1-momentum) * cur_mean
        self.temp_N = self.temp_N + cur_num

        self.iter = self.iter + 1
        if (self.iter % self.statistic_cfg['update_per_iter']) == 0:
            print('update!')
            self.iter = torch.zeros_like(self.iter)
            self.update_obj_params()

    def update_obj_params(self):
        self.obj_mean = self.statistic_cfg['momentum'] * self.obj_mean + (1 - self.statistic_cfg['momentum']) * self.temp_mean
        self.obj_var = self.statistic_cfg['momentum'] * self.obj_var + (1 - self.statistic_cfg['momentum']) * self.temp_var
    ...
    def create_dist_mask(self, bev_pos):
        bev_pos[..., 0] = bev_pos[..., 0] * self.test_cfg['out_size_factor'] * self.test_cfg['voxel_size'][0] + self.test_cfg['pc_range'][0]
        bev_pos[..., 1] = bev_pos[..., 1] * self.test_cfg['out_size_factor'] * self.test_cfg['voxel_size'][1] + self.test_cfg['pc_range'][1]
        bev_dist = torch.linalg.norm(bev_pos, dim=-1)
        near_mask = (bev_dist < 30)
        mid_mask = (bev_dist >= 25) & (bev_dist < 55)
        far_mask = (bev_dist >= 50)
        return near_mask, mid_mask, far_mask

    def init_weights(self):
        if self.statistic_cfg is not None:
            nn.init.xavier_uniform_(self.cls_emb)
            nn.init.xavier_uniform_(self.dist_emb)
            nn.init.xavier_uniform_(self.iou_emb)
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.statistic_cfg is not None:
            obj_emb = self.get_obj_emb()
            if self.statistic_cfg['sampling']:
                obj_features = self.sample_obj_features(batch_size)
                obj_emb = obj_emb.view(1, self.num_classes, self.ndist, self.niou, 1, -1).repeat(batch_size, 1, 1, 1, self.statistic_cfg['n_sample'], 1)
            else:
                obj_features = self.obj_mean.clone().unsqueeze(0).repeat(batch_size, 1, 1, 1, 1).unsqueeze(-2)
                obj_emb = obj_emb.view(1, self.num_classes, self.ndist, self.niou, -1).repeat(batch_size, 1, 1, 1, 1).unsqueeze(-2)

            bev_emb = self.bev_emb(bev_pos)
            
        
        if self.statistic_cfg is not None and self.statistic_cfg['pre_classification']:
            cls_scores = self.cls_head(lidar_feat)
            scores = cls_scores.detach().sigmoid().view(batch_size, cls_scores.shape[1], -1)
            final_preds = scores.max(1, keepdims=False).indices
            #final_scores = scores.max(1, keepdims=False).values
            for i in range(self.statistic_cfg['n_layers']):
                
                lidar_feat_flatten = self.encoder[i](lidar_feat_flatten, obj_features, bev_emb.transpose(1, 2), obj_emb, scores.transpose(1,2), final_preds)
                #print(lidar_feat_flatten[0, :, 0])
            #print(lidar_feat_flatten.shape)
            lidar_feat = lidar_feat_flatten.view(*(lidar_feat.shape))
        ...
        if self.initialize_by_heatmap:
            ...
            if self.training:
                temp_features = query_feat.clone().detach()
        ...
        if self.initialize_by_heatmap:
            ...
            if self.fuse_img:
                ...
                if self.training:
                    ret_dicts[0]['features'] = temp_features
                if (self.statistic_cfg is not None) and (self.statistic_cfg['pre_classification']):
                    ret_dicts[0]['pre_heatmap'] = cls_scores
        ...
        for key in ret_dicts[0].keys():
            if key not in ['dense_heatmap', 'dense_heatmap_old', 'query_heatmap_score', 'pre_heatmap']:
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        #print(ious.shape)
        self.update_temp_params(preds_dicts, ious)
        ...
        if self.initialize_by_heatmap:
            ...
            if 'pre_heatmap' in preds_dict:
                #print('pre_class!')
                loss_pre = self.loss_heatmap(clip_sigmoid(preds_dict['pre_heatmap']), heatmap, avg_factor=max(heatmap.eq(1).float().sum().item(), 1))
                loss_dict['loss_pre'] = loss_pre
