...
class Decoder(nn.Module):
    def __init__(self, head, in_channels, num_conv, num_bins, classes, num_decoder, head_conv=64, final_kernel=1,
                 conv_cfg=dict(type='Conv1d'),
                 norm_cfg=dict(type='BN1d'),
                 bias='auto'):
        ...
        self.embed =  nn.Parameter(torch.randn(head_conv, num_bins, classes), requires_grad=True)
        decoder = nn.ModuleList()
        for i in range(num_decoder):
            decoder.append(ContrastiveFeatureHead(head_conv, classes))
        if self.head == 'heatmap':
            self.embed_layer = build_conv_layer(
                    dict(type='Conv1d'),
                    num_bins,
                    head_conv,
                    kernel_size=1,
    ...
    def init_weights(self):
        kaiming_init(self.embed_layer)
        for m in self.conv_layers.modules():
            if isinstance(m, nn.Conv1d):
                kaiming_init(m)


    def forward(self, x, preds=None):
        ret_dict = {}
        batch_size = x.shape[0]
        head_related_features = self.conv_layers(x)
        statistic_embed = self.embed
        if preds is not None: 
            head_embed = self.embed_layer(preds)
        else:
            head_embed = None
        ...
        for i in range(len(self.decoder)):
            statistic_embed, sims = self.decoder[i](statistic_embed, self.with_embed(head_related_features, head_embed))
        ret_dict[self.head+'_sims'] = sims
        
        return statistic_embed, ret_dict
...
class CrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, bias=True):
        ...
        self.in_proj_y = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.in_proj_vx = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.in_proj_vx = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.out_proj_x = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.out_proj_y = nn.Linear(embed_dim, embed_dim, bias=bias)
        self._reset_parameters()
    ...
    def forward(self, x, y):
        ...
        p = self.in_proj_x(x)
        q = self.in_proj_y(y)
        vx = self.in_proj_vx(x)
        vy = self.in_proj_vy(y)
        p = p.view(B, N, self.num_heads, self.head_dim)
        q = q.view(B, N, self.num_heads, self.head_dim)
        vx = vx.view(B, N, self.num_heads, self.head_dim)
        vy = vy.view(B, N, self.num_heads, self.head_dim)

        prob = torch.einsum('bnhc,bmhc->bnmh', p, q)
        prob_x = torch.softmax(prob, dim=1)
        prob_y = torch.softmax(prob, dim=2)
        out_x = torch.einsum('bnmh,bnhc->bmhc', prob_x, vx)
        out_y = torch.einsum('bnmh,bmhc->bnhc', prob_y, vy)

        out_x = self.out_proj_x(out_x)
        out_y = self.out_proj_y(out_y)
        return out_x, out_y

class CrossModalLayer(nn.Module):
    def __init__(self, embed_dim, num_heads, ffn_channel, dropout=0.0, activation='relu', bias=True):
        super().__init__()
        self.cross_attn = CrossAttention(embed_dim, num_heads, bias=bias)
    ...
    def forward(self, x, x_img):
        ...
        """
        B, C, N, D = x.shape
        x = x.permute(0, 3, 2, 1).contiguous().view(B*D, N, C)
        x_img = x_img.permute(0, 3, 2, 1).contiguous().view(B*D, N, C)
        
        feat_x, feat_img = self.cross_attn(x, x_img)

        out_x = x + self.dropout2(feat_img)
        out_x = self.norm2(out_x)
        features2 = self.linear2(self.dropout(self.activation(self.linear1(out_x))))
        out_x = out_x + self.dropout3(features2)
        out_x = self.norm3(out_x)
        out_x = out_x.view(B, D, N, C).permute(0, 3, 2, 1)

        out_img = x_img + self.dropout2(feat_x)
        out_img = self.norm2(out_img)
        features2 = self.linear2(self.dropout(self.activation(self.linear1(out_img))))
        out_img = out_img + self.dropout3(features2)
        out_img = self.norm3(out_img)
        out_img = out_img.view(B, D, N, C).permute(0, 3, 2, 1)
        return out_x, out_img
...
class SFFN(nn.Module):
    def __init__(self,
        ...
        kernel = kwargs.get('kernel', 'dot')
        unit = kwargs.get('unit', True)
        self.fuse_img = kwargs.get('fuse_img', False)
        self.cross_modal_fuse = kwargs.get('cross_modal_fuse', False)
        for head in self.heads:
            classes, num_conv, num_bins, num_encoder, num_decoder, num_heads, ffn_channel, activation = self.heads[head]

            if head == 'heatmap':
                self.__setattr__(head+'_decoder', Decoder(head, in_channels, num_conv, num_bins+1, classes, num_decoder, head_conv))
            else:
                self.__setattr__(head+'_decoder', Decoder(head, in_channels, num_conv, num_bins, classes, num_decoder, head_conv))

            encoder = nn.ModuleList()
            for i in range(num_encoder):
                encoder.append(
                    GPTransformerLayer(
                        head_conv, num_heads, ffn_channel, dropout=0.1, activation=activation,
                        self_posembed=None, kernel=kernel, unit=unit, use_gt=self.use_gt
                    )
                )
            ...
            if self.fuse_img:
                if head == 'heatmap':
                    self.__setattr__('img_'+head+'_decoder', Decoder(head, in_channels, num_conv, num_bins+1, classes, num_decoder, head_conv))
                else:
                    self.__setattr__('img_'+head+'_decoder', Decoder(head, in_channels, num_conv, num_bins, classes, num_decoder, head_conv))
                ...
                self.__setattr__('img_'+head+'_encoder', encoder)

            if self.cross_modal_fuse:
                self.fusion_layer = CrossModalLayer(num_conv, num_heads, ffn_channel, dropout=0.1, activation=activation)

            if head == 'heatmap':
                out_layer = build_conv_layer(dict(type='Conv1d'), head_conv, num_bins, kernel_size=final_kernel, stride=1, padding=final_kernel // 2, bias=True)
                self.__setattr__(head, out_layer)
                if self.fuse_img:
                    out_layer = build_conv_layer(dict(type='Conv1d'), head_conv, num_bins, kernel_size=final_kernel, stride=1, padding=final_kernel // 2, bias=True)
                    self.__setattr__('img_'+head, out_layer)
                if self.use_gt:
                    gt_layer = build_conv_layer(dict(type='Conv1d'), num_bins, head_conv, kernel_size=1, stride=1, bias=True)
                    self.__setattr__(head+'_gt_layer', gt_layer)
            else:
                out_layer = build_conv_layer(dict(type='Conv1d'), head_conv*classes, classes, kernel_size=final_kernel, stride=1, padding=final_kernel // 2, groups=classes, bias=True)
                self.__setattr__(head, out_layer)
                if self.fuse_img:
                    out_layer = build_conv_layer(dict(type='Conv1d'), head_conv*classes, classes, kernel_size=final_kernel, stride=1, padding=final_kernel // 2, groups=classes, bias=True)
                    self.__setattr__('img_'+head, out_layer)
                if self.use_gt:
                    gt_layer = build_conv_layer( dict(type='Conv1d'), classes, head_conv, kernel_size=1, stride=1, bias=True)
                    self.__setattr__(head+'_gt_layer', gt_layer)
    ...
    def forward(self, x, img_x, preds_dict=None, gt_head_dict=None):
        """Forward function for SepHead.
        ...
        for head in self.heads:
            if preds_dict is not None: 
                preds = preds_dict[head]
            else:
                preds = None
            statistic_embed, out_dict = self.__getattr__(head+'_decoder')(x, preds)
            ret_dict.update(out_dict)
            if self.fuse_img:
                img_statistic_embed, img_dict = self.__getattr__('img_'+head+'_decoder')(img_x, preds)
                ret_dict.update(img_dict)
            
            if self.cross_modal_fuse:
                statistic_embed, img_statistic_embed = self.fusion_layer(statistic_embed, img_statistic_embed)
            ...
            if gt_head_dict is not None:
                gt_embed = self.__getattr__(head+'_gt_layer')(gt_head_dict[head])
            else:
                gt_embed = None
            for i in range(len(self.__getattr__(head+'_encoder'))):
                #is_last = (i==(len(self.__getattr__(head+'_encoder'))-1))
                is_last = False
                head_related_features, sigma, head_related_features_p, sigma_p = self.__getattr__(head+'_encoder')[i](statistic_embed, 
                                                                            self.with_embed(head_related_features, head_embed), gt_embed, num_classes)
            
            
            #print(torch.isnan(head_related_features).sum())
            ret_dict[head+'_mean'] = self.__getattr__(head)(head_related_features)
            ret_dict[head+'_sigma'] = sigma
            if head_related_features_p is not None:
                ret_dict[head+'_p'] = self.__getattr__(head)(head_related_features_p)
                ret_dict[head+'_sigma_p'] = sigma_p

            
        return ret_dict

class GaussianProcess(nn.Module):
    def __init__(self, embed_dim, num_heads, bias=True, kernel='dot', unit=False, noise=False, use_gt=False):
        ...
        self.noise = None
        if noise:
            self.noise = nn.Parameter(torch.FloatTensor([0]), requires_grad=True)
        self.use_gt = use_gt 
        if self.use_gt:
            self.in_proj_q_ = nn.Linear(embed_dim*2, embed_dim, bias=bias)
            self.in_proj_k_ = nn.Linear(embed_dim, embed_dim, bias=bias)
    ...
    def forward_(self, q, k, v, num_classes):
        batch_size = q.shape[0]
        N = q.shape[1]
        if self.kernel == 'dot':
            if self.unit:
                sim = torch.einsum('bnhc,bmhc->bnmh', q, k)
            else:
                sim = torch.einsum('bnmhc,bmhc->bnmh', q.unsqueeze(2)-k.unsqueeze(1), k)
            sim = sim.view(batch_size, N, -1, num_classes, self.num_heads)
            sim = torch.softmax(sim * self.scale, dim=2)
            if self.unit:
                sigma = 1 - torch.sum(torch.pow(sim, 2), dim=2)
            else:
                Kqq = torch.linalg.norm(q, dim=-1).view(batch_size, N, 1, self.num_heads)
                Kkk = torch.linalg.norm(k, dim=-1).view(batch_size, 1, -1, num_classes, self.num_heads)
                sigma = Kqq - torch.sum(sim / torch.clamp(Kkk, min=1e-3), dim=2)
                sigma = torch.clamp(sigma, min=1e-3)
            sigma = sigma.mean(dim=-1)
        elif self.kernel == 'exp':
            sim = torch.linalg.norm(q.unsqueeze(2)-k.unsqueeze(1), dim=-1)
            sim = sim.view(batch_size, N, -1, num_classes, self.num_heads)
            sim = torch.softmax(-sim * self.scale, dim=2)
            sigma = 1 - torch.sum(torch.pow(sim, 2), dim=2)
            sigma = sigma.mean(dim=-1)
            ...
            sim = torch.linalg.norm(q.unsqueeze(2)-k.unsqueeze(1), dim=-1)
            sim = sim.view(batch_size, N, -1, num_classes, self.num_heads)
            sim = torch.softmax(-(sim * self.scale).pow(2), dim=2)
            sigma = 1 - torch.sum(torch.pow(sim, 2), dim=2)
            sigma = sigma.mean(dim=-1)

        #print(sim.shape, v.shape)
        out = torch.einsum('bnmkh,bmkhc->bnkhc', sim, v).contiguous().view(batch_size, N, num_classes, -1)
        #out = torch.sum(out, dim=2)
        out = self.out_proj(out)
        return out, sigma

    def forward(self, head_embed, features, gt_embed, num_classes):
        batch_size = head_embed.shape[0]
        q = self.in_proj_q(features)
        k = self.in_proj_k(head_embed)
        q = q.view(batch_size, -1, self.num_heads, self.head_dim)
        k = k.view(batch_size, -1, self.num_heads, self.head_dim)
        v = self.in_proj_v(head_embed)
        v = v.view(batch_size, -1, num_classes, self.num_heads, self.head_dim)
        out, sigma = self.forward_(q, k, v, num_classes)
        if self.use_gt and (gt_embed is not None):
            q_ = self.in_proj_q_(torch.cat([features, gt_embed], dim=-1))
            k_ = self.in_proj_k_(head_embed)
            q_ = q_.view(batch_size, -1, self.num_heads, self.head_dim)
            k_ = k_.view(batch_size, -1, self.num_heads, self.head_dim)
            out_, sigma_ = self.forward_(q_, k_, v, num_classes)
            return out_, sigma_, out, sigma
        return out, sigma, None, None

class GPTransformerLayer(nn.Module):
    def __init__(self, embed_dim, num_heads, ffn_channel, dropout=0.0, activation='relu', self_posembed=None, bias=True, kernel='dot', unit=True, use_gt=False):
        super().__init__()
        self.gp = GaussianProcess(embed_dim, num_heads, bias=bias, kernel=kernel, unit=unit, noise=False, use_gt=use_gt)
    ...
    def forward(self, head_embed, features, gt_embed, num_classes):
        ...
        if gt_embed is not None:
            gt_embed = gt_embed.transpose(1, 2)
        
        feat, sigma, feat_p, sigma_p = self.gp(head_embed, self.with_pos_embed(features, pos_emb), gt_embed, num_classes)

        x = features.unsqueeze(2) + self.dropout2(feat)
        x = self.norm2(x)
        ...
        x = x.view(B, N, -1).transpose(1, 2)
        sigma = sigma.transpose(1, 2)

        if feat_p is not None:
            x_ = features.unsqueeze(2) + self.dropout2(feat_p)
            x_ = self.norm2(x_)

            features2 = self.linear2(self.dropout(self.activation(self.linear1(x_))))
            x_ = x_ + self.dropout3(features2)
            x_ = self.norm3(x_)

            x_ = x_.view(B, N, -1).transpose(1, 2)
            sigma_ = sigma_p.transpose(1, 2)
        else:
            x_ = None
            sigma_ = None

        return x, sigma, x_, sigma_
...
class STransFusionHeadv3(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(STransFusionHeadv3, self).__init__()
        ...
        self.use_gt = statistic_cfg.get('use_gt', False)
        kernel = statistic_cfg.get('kernel', 'dot')
        unit = statistic_cfg.get('unit', True)
        if self.initialize_by_heatmap:
            if statistic_cfg['heatmap_head']:
                ...
                self.headmap_head = SFFN(hidden_channel, head, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias, use_gt=False, kernel=kernel, unit=unit)
            else:
                layers = []
        ...
        for i in range(self.num_decoder_layers):
            ...
            self.prediction_heads.append(SFFN(hidden_channel, heads, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias, use_gt=self.use_gt, kernel=kernel, unit=unit))
    ...
    def forward_single(self, inputs, img_inputs, img_metas, gt_bboxes_3d=None, gt_labels_3d=None):
        ...
        if self.initialize_by_heatmap:
            if self.statistic_cfg['heatmap_head']:
                ...
                gt_dict = None
                init_dict = self.headmap_head(dense_heatmap_flatten, bev_pos, None, gt_dict)
                #print(dense_heatmap)
                dense_heatmap = init_dict['heatmap_mean']
        ...
        if self.initialize_by_heatmap:
            ...
            if self.statistic_cfg['heatmap_head'] is not None:
                ret_dicts[0]['dense_heatmap_sigma'] = init_dict['heatmap_sigma']
            if self.fuse_img:
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        for batch_idx in range(len(gt_bboxes_3d)):
            ...
            for key in preds_dict[0].keys():
                if (key not in ['gt_dict']) and ('mean' not in key) and ('var' not in key) and ('basis' not in key):
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        if self.initialize_by_heatmap:
            ...
            if self.statistic_cfg['heatmap_head'] is not None:
                loss_dict['loss_heatmap_sigma'] = torch.mean(preds_dict['dense_heatmap_sigma'])
        ...
        for idx_layer in range(self.num_decoder_layers if self.auxiliary else 1):
            ...
            if self.use_gt:
                ...
                layer_center_var = preds_dict['center_sigma_p'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                layer_height_var = preds_dict['height_sigma_p'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                layer_rot_var = preds_dict['rot_sigma_p'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                layer_dim_var = preds_dict['dim_sigma_p'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
                preds_var = torch.cat([layer_center_var, layer_height_var, layer_dim_var, layer_rot_var], dim=1).permute(0, 2, 1)
                loss_dict[f'{prefix}_loss_reg_sigma_p'] = torch.sum(preds_var * layer_reg_weights) / max(num_pos, 1) * 0.2
            ...
            layer_center_var = preds_dict['center_sigma'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
            layer_height_var = preds_dict['height_sigma'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
            layer_rot_var = preds_dict['rot_sigma'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
            layer_dim_var = preds_dict['dim_sigma'][..., idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals]
            preds_var = torch.cat([layer_center_var, layer_height_var, layer_dim_var, layer_rot_var], dim=1).permute(0, 2, 1)
            loss_dict[f'{prefix}_loss_reg_sigma'] = torch.sum(preds_var * layer_reg_weights) / max(num_pos, 1) * 0.2
            ...
            for head in ['center', 'height', 'dim', 'rot']:
                ...
                loss_dict[f'{prefix}_{head}_dist'] = self.loss_dist(preds_dict[head+'_attn_map_x'], preds_dict[head+'_attn_map_xy'], targets_dict[head+'_weights'][..., 0:1], avg_factor=max(num_pos, 1))
            '''
            ...
            loss_dict[f'{prefix}_loss_cls_sigma'] = torch.mean(preds_dict['heatmap_sigma']) * 0.2
            ...
            if self.use_gt:
                loss_dict[f'{prefix}_loss_pbox'] = layer_loss_pbox
                loss_dict[f'{prefix}_loss_cls_sigma_p'] = torch.mean(preds_dict['heatmap_sigma_p']) * 0.2
