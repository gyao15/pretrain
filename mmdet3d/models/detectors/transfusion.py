...
class TransFusionDetector(MVXTwoStageDetector):
    ...
    def simple_test_pts(self, x, x_img, img_metas, rescale=False):
        ...
        bbox_results = []
        for i, x in enumerate(bbox_list):
            bbox_results.append(bbox3d2result(x[0], x[1], x[2]))
            if self.store:
                bbox_results[i].update({'features': x[3].cpu()})
            if self.store_box:
                bbox_results[i].update({'box_features': x[4].cpu()})
        return bbox_results
