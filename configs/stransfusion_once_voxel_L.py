...
out_size_factor = 8
evaluation = dict(interval=1, out_dir='./work_dirs/transfusion_once_voxel_L/val_results')
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        statistic_cfg=dict(sampling=False, n_sample=1, iou=['fp', 'inacc', 'pos'], dist=['near', 'mid', 'far'], 
                           preload_path='work_dirs/transfusion_once_voxel_L/train_results/', n_layers=1,
                           proj_class_agnostic=True, attn_class_agnostic=False, ffn_ratio=2, pre_classification=True,
                           iou_threshold=[0.7, 0.7, 0.7, 0.5, 0.3], update_per_iter=1000, momentum=0.1)
    ),
    train_cfg=dict(
        pts=dict(
            dataset='once',
