...
out_size_factor = 8
evaluation = dict(interval=1, jsonfile_prefix='work_dirs/bevfusion_nusc_voxel_LC/')
...
train_pipeline = [
    ...
    dict(type='ImageAug3D', final_dim=[256, 704], resize_lim=[0.48, 0.48], bot_pct_lim=[0.0, 0.0], rot_lim=[0, 0], is_train=False),
    dict(type='MyNormalize', **img_norm_cfg),
    dict(type='DefaultFormatBundle3D', class_names=class_names),
    dict(type='Collect3D', keys=['points', 'raw_points', 'img', 'gt_bboxes_3d', 'gt_labels_3d'])
]
test_pipeline = [
    dict(
        type='LoadPointsFromFile',
    ...
    dict(
        ...
        transforms=[
            ...
            dict(
                ...
                with_label=False),
            dict(type='Collect3D', keys=['points', 'raw_points', 'img'])
        ])
]
data = dict(
    samples_per_gpu=2,
...
model = dict(
    ...
    img_backbone=dict(
        ...
        #init_cfg=dict(type='Pretrained', checkpoint='work_dirs/swin_tiny_patch4_window7_224.pth'),
        ),
    img_neck=dict(
...
work_dir = None
load_from = None
resume_from = 'work_dirs/bevfusion_nusc_voxel_LC/epoch_15.pth'
workflow = [('train', 1)]
gpu_ids = range(0, 5)
