...
out_size_factor = 8
evaluation = dict(interval=1, out_dir='./work_dirs/transfusion_once_voxel_L/train_results')
...
model = dict(
    ...
    geometry_cfg=dict(
        ...
        out_size_factor=out_size_factor,
        assigner=dict(
                type='HungarianAssigner3D_v2',
                iou_calculator=dict(type='BboxOverlaps3D', coordinate='lidar'),
                cls_cost=dict(type='KLCost', weight=0.15),
                reg_cost=dict(type='BBoxBEVL1Cost', weight=0.25),
                iou_cost=dict(type='IoU3DCost', weight=0.25)
            ),
    ))
...
work_dir = None
load_from = None
resume_from = 'work_dirs/ctransfusion_nusc_voxel_L/latest.pth'
workflow = [('train', 1)]
gpu_ids = range(0, 6)
